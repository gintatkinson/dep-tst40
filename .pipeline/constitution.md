...
- A single YANG module maps to a single Epic unless the module defines >15 distinct schema nodes requiring Feature extraction, at which point splitting is warranted. Internal container, choice, or case nesting within a single module grouping does NOT justify separate Epics. The same principle applies to OpenAPI paths, Protobuf services, and other schema formats — one module = one Epic until the Feature count exceeds the upper bound.
